...
function renderNowPlaying(t) {
  ...
  for (const k of ['heart','fire','hundred','down','prayer']) {
  ...
  for (let i = 0; i < visible.length; i++) {
    ...
    const reactionHtml = rb.length
      ? `<span class="react-pills">${rb.join(' ')}</span>
         <button class="row-mute${isMuted ? ' active' : ''}" data-mid="${t.id}" title="${isMuted ? 'Unmute: play this track' : 'Mute: skip during playback'}">🔇</button>`
      : `<span class="react-mini">
          <button class="react-sm" data-tid="${t.id}" data-rtype="heart" title="Love it">❤️</button>
          <button class="react-sm" data-tid="${t.id}" data-rtype="fire" title="Fire!">🔥</button>
          <button class="react-sm" data-tid="${t.id}" data-rtype="hundred" title="100%">💯</button>
          <button class="react-sm" data-tid="${t.id}" data-rtype="prayer" title="Praying hands">🙏</button>
        </span>
        <button class="row-mute${isMuted ? ' active' : ''}" data-mid="${t.id}" title="${isMuted ? 'Unmute: play this track' : 'Mute: skip during playback'}">🔇</button>`;

    // Per-track links — gray currentColor, brand shapes
    const ytSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>';
    const spSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>';
    const amSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" d="M23.994 6.124a9.23 9.23 0 00-.24-2.19c-.317-1.31-1.064-2.31-2.19-3.09a5.33 5.33 0 00-1.72-.77c-.63-.15-1.26-.19-1.9-.19H6.026c-.55 0-1.1.04-1.64.13C3.14.19 2.19.78 1.38 1.67.65 2.49.17 3.49.04 4.61A11.17 11.17 0 000 6.01v11.96c0 .51.04 1.01.12 1.51.2 1.24.78 2.26 1.69 3.08.82.74 1.81 1.18 2.91 1.35.55.09 1.1.11 1.66.11h11.29c.6 0 1.2-.04 1.79-.15 1.07-.19 2.01-.65 2.79-1.38.87-.81 1.42-1.82 1.6-3 .1-.59.14-1.19.14-1.79V6.12zm-6.97 9.26c0 .14-.01.28-.04.42a1.8 1.8 0 01-.87 1.25c-.27.16-.57.24-.88.24-.66 0-1.28-.37-1.64-.93l-.02-.03-3.43-5.93v6.57c0 .12-.01.25-.03.37a1.79 1.79 0 01-1.77 1.48 1.79 1.79 0 01-1.78-1.78V8.62c0-.12.01-.25.03-.37A1.79 1.79 0 018.38 6.77c.66 0 1.28.37 1.64.93l.02.03 3.43 5.93V7.09c0-.12.01-.25.03-.37A1.79 1.79 0 0115.27 5.3c.99 0 1.79.8 1.79 1.79v8.27z"/></svg>';
    const rowLinks = [];
    if (t.youtube_id)  rowLinks.push('<a class="row-link" href="https://www.youtube.com/watch?v=' + esc(t.youtube_id) + '" target="_blank" rel="noopener" title="YouTube">' + ytSvg + '</a>');
    if (t.spotify_url) rowLinks.push('<a class="row-link" href="' + esc(t.spotify_url) + '" target="_blank" rel="noopener" title="Spotify">' + spSvg + '</a>');
    if (t.apple_url)   rowLinks.push('<a class="row-link" href="' + esc(t.apple_url) + '" target="_blank" rel="noopener" title="Apple Music">' + amSvg + '</a>');
      ...
      <td>
        <div class="track-cell">
          ...
          <div class="track-info">
            ...
            <div class="track-meta-mobile">${esc(abbrevName(t.shared_by) || '')} · ${fmtDate(t.shared_at || t.added_at)}</div>
          </div>
        </div>
      </td>
    ...
    tr.addEventListener('click', e => {
      if (e.target.closest('.react-sm') || e.target.closest('.row-link') || e.target.closest('.drag-handle') || e.target.closest('.hide-btn') || e.target.closest('.row-mute')) return;
...
// ── Per-row mute toggle ──────────────────────────────────────
document.getElementById('catalog-table').addEventListener('click', e => {
  const btn = e.target.closest('.row-mute');
  if (!btn) return;
  e.stopPropagation();
...
(function setupAppleMusic() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (title) title.textContent = 'Playlist Added!';
      if (modalStatus) modalStatus.innerHTML = `“Music Fellowship” has been added to your Apple Music library with ${result.added} songs.<br><br><span style="color:#8e8e93;font-size:0.75rem">Open Apple Music → Library → Playlists</span>`;
      if (actions) actions.innerHTML = `
        <a href="https://music.apple.com/library/playlist/${result.playlistId || ''}" 
           ...
           target="_blank">Open in Apple Music</a><br>
